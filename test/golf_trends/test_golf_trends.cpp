...
std::string row(const uint8_t holes, const uint16_t strokes, const uint16_t par, const uint16_t putts,
  ...
  char output[160];
  snprintf(output, sizeof(output), ",Course,%u,%u,%u,%u,%u,%u,round.json\r\n", holes, strokes, par, putts, in100,
           out100);
  return output;
}
...
TEST(GolfTrends, BestAndWorstHandleSingleRoundAndTies) {
  ...
  const GolfTrendStats ties = calculate(row(18, 80, 72, 30, 50, 30) + row(18, 74, 72, 28, 44, 30) +
                                         row(18, 91, 72, 35, 61, 30) + row(18, 74, 72, 29, 44, 30) +
                                         row(18, 91, 72, 36, 61, 30));
