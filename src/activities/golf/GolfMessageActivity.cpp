...
void GolfMessageActivity::render(RenderLock&&) {
  ...
  const auto layout = golfui::chromeLayout(renderer, metrics.topPadding, metrics.headerHeight,
                                            freeink::ui::Insets{12, 24, 12, 24});
  renderer.clearScreen();
  GUI.drawHeader(renderer, Rect{layout.header.x, layout.header.y, layout.header.width, layout.header.height}, title);
  const int lineHeight = renderer.getLineHeight(UI_10_FONT_ID);
  UITheme::drawCenteredText(renderer,
                            Rect{layout.body.x, layout.body.y, layout.body.width, layout.body.height}, UI_10_FONT_ID,
                            layout.body.y + (layout.body.height - lineHeight) / 2, message);
